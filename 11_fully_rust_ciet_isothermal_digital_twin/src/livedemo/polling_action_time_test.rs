...
pub fn demo_5_polling_action_time(run_server: bool) {
    ...
    let nine_second_timer = move || {
        let dur = time::Duration::from_millis(1000);
        thread::sleep(dur);

        println!("{}s reached!", dur.as_secs());


    };

    server.add_polling_action(9000, nine_second_timer);
